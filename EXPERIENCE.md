...
核心规则：**永远不会把带 `{xxx}` 占位符的半成品复制出去**。

- 参数都已填（绿色值还在）→ **点击行任意位置或 Copy → 立即复制**，和无参命令一样直接。
- 参数没填完 → 点击行/Copy **不复制**，自动进入编辑并聚焦第一个空参数。
- 命令里的 `{peer}`、`{interface}` 等占位符橙色显示，**点击占位符 → 原地变成输入框**直接输入。
- 多参数模板：所有占位符同时变为输入框，焦点落在你点的那个。
- 同名参数出现多次：只输入一次，替换所有同名占位符。
- **Enter**：全部填完 → 值填进命令行（绿色）并复制；还有空的 → 聚焦第一个空参数，不复制。
- `Esc` 取消编辑，已输入的值保留，下次点击还在；已填的绿色值可再点修改。
...
| v0.2.1 | 有参命令点击即复制（未填参数保留占位符），点占位符才进入编辑；修复空参数会把占位符替换成空的问题 |
| v0.2.2 | 未填完的参数命令不再复制半成品：点击进入编辑并聚焦第一个空参数，Enter 有空参数也只聚焦不复制 |
| v0.2.3 | 修复 1.2 秒内连续复制两次导致按钮卡在 ✓ Copied 的竞态（恢复文本写死 + 清除旧定时器） |
