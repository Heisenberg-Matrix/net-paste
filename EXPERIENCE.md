...
- **点击行任意位置或 Copy → 立即复制**，未填的参数保留 `{xxx}` 占位符原样（和复制无参命令一样直接，不需要知道任何“先回车”的规则）。
- 命令里的 `{peer}`、`{interface}` 等占位符橙色显示，**点击占位符 → 原地变成输入框**直接输入。
- 多参数模板：所有占位符同时变为输入框，焦点落在你点的那个；再点行内其他位置聚焦第一个还没填的。
- 同名参数出现多次：只输入一次，替换所有同名占位符。
- **Enter →** 值填进命令行（绿色）并复制；点 Copy / 点行也一样复制当前值。
- `Esc` 取消编辑，已输入的值保留，下次点击还在；已填的绿色值可再点修改。
- 编辑中点行其他区域或 Copy = 用当前已输入的值立即复制并结束编辑。
